...
class MainPage(BaseDriveHandler):
  ...
  def get(self, *args):
    ...
    if not creds:
        return self.redirect(self.RedirectAuth())
...
class AuthHandler(BaseDriveHandler):
    def get(self):
        ...
        if not creds:
            return self.RespondJSON({'redirectUri': self.RedirectAuth()})
...
app = webapp2.WSGIApplication(
    [(r'/', MainPage), (r'/edit/(\w*)', MainPage), (r'/courses', CoursesHandler), (r'/svc', ServiceHandler), (r'/about', AboutHandler),
     (r'/auth', AuthHandler) ,(r'/user', UserHandler)],
